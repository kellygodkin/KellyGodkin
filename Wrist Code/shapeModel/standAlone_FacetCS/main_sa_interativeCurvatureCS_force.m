function bone_acs=main_sa_interativeCurvatureCS(facetIVpath,boneIVpath,writeIVfile_flag,threshR, boneCS_opt, verbose)
...
avg_1 = calcAvgMotion(ham_x);
avg_2 = calcAvgMotion(ham_y);
...
%     bone_acs = [-bone_acs(1:3,3) bone_acs(1:3,2) bone_acs(1:3,1) old_saddle'; 0 0 0 1];



% % % % 
% % % %     %%%HARDCODESHIFT FOR PILOT 625 ONLY
% % % %     old_saddle=[12.4, 4.4, 64.9];
% % % %     
    ...
    figure;
    hold on
    trisurf(con1(:,1:3)+1,pts1(:,1),pts1(:,2),pts1(:,3),'linestyle','none');
    plot_CS(bone_acs);
    axis equal
