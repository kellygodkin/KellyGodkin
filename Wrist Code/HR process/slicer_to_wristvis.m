...
%% Save Files in .dat structures

% maindir = 'P:\Data\2025-04-09 Carpometacarpal Pilot';
% for s = 1: length(subject)
%     mkdir(maindir,subject{s})
% 
% 
% 
% end
...
for s = 1:length(subject)
    for p = 1%:length(positions)
        ...
        for b = 1:numel(fields(bonestruct))
            ...
            filepath = fullfile(filepath_base,subject{s},'Models\IV\original',filename);
            
            ivstring = [ivstring createInventorLink(filepath, bonestruct.(bonecode_hand(b))(s).transforms.(positions{p})(1:3,1:3), bonestruct.(bonecode_hand(b))(s).transforms.(positions{p})(1:3,4)', [0.7,0.7,0.7], 0.5)];
