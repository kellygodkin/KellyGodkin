...
for s = 1:length(subject)
    ...
    for b = bones
        ...
        bonestruct.(bonecode_hand(b))(s).acs.cmccs = acs;
...
%% Create scaphoid tpm CS
% Variables
bones = [3, 8]; % The bones you want to get a surface coordinate system 
...
for s = 1:length(subject)
    ivstring = createInventorHeader();
    for b = bones
        facetIVfname = [subject{s} '_' bonecode_hand(b) '_surface_st.iv'];
        facetIVpath = fullfile(filepath_base, 'preprocessing',subject{s},'Models\IV\surface',facetIVfname);
        boneIVfname = [subject{s} '_' bonecode_hand(b) '_R.iv'];
        boneIVpath = fullfile(filepath_base, 'preprocessing',subject{s},'Models\IV\original',boneIVfname);
        inertiaFile = dlmread(fullfile(filepath_base,subject{s},'S15R','inertia15R.dat'));
        ...
        acs = main_sa_interativeCurvatureCS(facetIVpath,boneIVpath,writeIVfile_flag, boneCS_opt, verbose);  
        bonestruct.(bonecode_hand(b))(s).acs.stcs = acs;
    ...
    fid = fopen(fullfile(filepath,'sca_tpm_CS.iv'),"w");
    fprintf(fid,ivstring);
    fclose(fid);
end

% save(fullfile(filepath_base,'preprocessing\structures\bonestruct_CMCCS.mat'),"bonestruct");%% Create scaphoid tpm CS

%% Create scaphoid rad CS
% Variables
bones = [1, 3]; % The bones you want to get a surface coordinate system 
...
for s = 1:length(subject)
    ivstring = createInventorHeader();
    for b = bones
        facetIVfname = [subject{s} '_' bonecode_hand(b) '_surface_rs.iv'];
        facetIVpath = fullfile(filepath_base, 'preprocessing',subject{s},'Models\IV\surface',facetIVfname);
        boneIVfname = [subject{s} '_' bonecode_hand(b) '_R.iv'];
        boneIVpath = fullfile(filepath_base, 'preprocessing',subject{s},'Models\IV\original',boneIVfname);
        inertiaFile = dlmread(fullfile(filepath_base,subject{s},'S15R','inertia15R.dat'));
        ...
        acs = main_sa_interativeCurvatureCS(facetIVpath,boneIVpath,writeIVfile_flag, boneCS_opt, verbose);  
        bonestruct.(bonecode_hand(b))(s).acs.rscs = acs;
