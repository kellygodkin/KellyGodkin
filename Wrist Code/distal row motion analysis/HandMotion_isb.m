...
for s = 1:length(directory_neutral)
    ...
    directory_general = fullfile('C:\Users\kelly\Desktop\ISB_25',bonecode(bone_registered));

    hand = 'R';      %change per trial for all
    p1 = nchoosek(info.series(2:end-1),2); %moving from position 1 to pos 2
    p2 = nchoosek(info.series(1:end-1),2);
    ...
    for p = 1:length(p1)
        ...
        for b = 1:15
            ...
            ivstring = [ivstring createInventorLink(fullfile(info.neutralSeriesPath,filename_temp), boneregp2.(subject{s}).(p1{p,2}).(bonecode(b))(1:3,1:3), boneregp2.(subject{s}).(p1{p,2}).(bonecode(b))(1:3,4)', colour(b,:) )];
            ivstring = [ivstring createInventorArrow()]
        
            %Neutral original position
            ivstring = [ivstring createInventorLink(fullfile(info.neutralSeriesPath,filename_temp), boneregp1.(subject{s}).(p1{p,1}).(bonecode(b))(1:3,1:3), boneregp1.(subject{s}).(p1{p,1}).(bonecode(b))(1:3,4)', colour(b,:))]; %registered position (grey)
            % if b <11
...
% 
%         for b = 1:15
%             bone.phi_relative.(subject{s})(b,s) = bone.phi.(subject{s})(b,s) - bone.phi.(subject{s})(bone_registered,s);
% 
...
for p = 1:length(p2)
    for b = 1:15
        ...
        bone.phi_relative.(subject{s})(b,p) = (bone.phi.(subject{s})(b,p) - bone.phi.(subject{s})(bone_rotated,p));
...
phi_temp = all_phi(:,[1:37,38,40:45])
phi_mean = nanmean(phi_temp')';
abs_phi_mean = nanmean(abs(phi_temp'))';

phi_std = std(phi_temp')';
relativeAng_mean = nanmean(phi_diff')';
relativeAng_std = nanstd(phi_diff')';

figure(length(subject)+1);
hold on

% subplot(2,1,1)
boxplot(phi_diff',bonename);
...
for b = 1:length(boneindex)
    ...
    % bone.ttest(b,1) = nan; %'mean phi tpm'}
    ...
    combined(b+1,:) = [{bonecode(boneindex(b))}, bone.ttest.p(b,length(subject)+2), bone.ttest.sd(b,length(subject)+1), phi_mean(b,:),abs_phi_mean(b,:),mean_phi_diff(b,:), mean_phi_diff_abs(b,:)] ;
